...
const NFTList: FC = () => {
    ...
    useEffect(() => {
        const fetchNFTs = async () => {
            ...
            try {
                ...
                console.log('Processed worthless NFTs:', nftList);
                setNfts(nftList);
            } catch (error) {
    ...
    const handleScoop = async () => {
        ...
        try {
            setIsScooping(true);
            const transaction = await NFTService.prepareScoopTransaction(
            ...
            setNfts(updatedNFTs);
            setSelectedNFTs(new Set());
            console.log('NFTs scooped successfully!');
        } catch (error) {
    ...
    const renderScoopButton = () => {
        ...
        return (
            ...
            >
                ...
                {isScooping
                    ? 'Scooping...'
                    : `Scoop ${selectedNFTs.size} NFT${selectedNFTs.size !== 1 ? 's' : ''}`}
            </button>
        );
    };
            ...
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="bg-white/45 backdrop-blur-sm p-8 rounded-2xl shadow-xl border-[1.5px] border-white max-w-md w-full transform transition-all hover:scale-[1.02]">
                    <div className="flex flex-col items-center text-center">
                        ...
                        <p className="text-[#00243D]/70 text-lg max-w-sm">
                            Connect your wallet or switch to a wallet containing NFTs to start
                            scooping
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 ${
                            selectedNFTs.size > 0
                                ? 'bg-[#FC8E03] text-white'
                                : 'bg-white/60 text-[#FC8E03] hover:bg-[#FC8E03]/10'
                        }`}
                    >
                    ...
                    <div
                        ...
                        className={`group relative bg-white/45 rounded-lg overflow-hidden cursor-pointer transition-all duration-200 ${
                            selectedNFTs.has(nft.mint)
                                ? 'ring-2 ring-[#FC8E03] transform scale-[1.02]'
                                : 'hover:scale-[1.02]'
                        }`}
                    >
                        <div className="absolute top-1.5 right-1.5 z-10">
                            ...
                            >
                                <div
                                    className={`w-2.5 h-2.5 rounded-sm transition-transform ${
                                        selectedNFTs.has(nft.mint) ? 'bg-white' : 'bg-white/80'
                                    }`}
                                />
                            </div>
                        </div>
                            ...
                            <img
                                ...
                                onError={(e) => {
                                    (e.target as HTMLImageElement).src =
                                        '/images/bonk_logo_transparent.png';
                                }}
                            />
                        </div>
                        <div className="p-2 bg-white/60 backdrop-blur-sm">
                            <h3 className="font-medium text-[#00243D] text-xs truncate">
                                {nft.name}
                            </h3>
                        </div>
                    </div>
